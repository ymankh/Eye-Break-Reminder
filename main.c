...
static void FormatTime(int seconds, wchar_t *buffer, size_t buffer_count)
{
    ...
    (void)buffer_count;
    swprintf(buffer, L"%02d:%02d", minutes, remaining_seconds);
}
...
static void UpdateStatusText(void)
{
    ...
    swprintf(text, L"Next break in %s", time_text);
...
static void PaintBackground(HWND hwnd)
{
    ...
    swprintf(label, g_in_break ? L"BREAK IN PROGRESS" : L"BACKGROUND REMINDER");
